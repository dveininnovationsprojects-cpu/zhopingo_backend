...
exports.generateWeeklySettlement = async (req, res) => {
  try {
    ...
    try {
        ...
        // 1. Double check for existing settlement
        const existing = await Settlement.findOne({ sellerId: sellerObjectId, weekRange: `${startDate} to ${endDate}` });
        if (existing) return res.status(400).json({ success: false, message: "Settlement already exists!" });
        ...
        // 🚀 3. THE AGGREGATION: Directly fetch and calculate in one shot
        const orders = await Order.aggregate([
            ...
            { $unwind: "$sellerSplitData" },
            { $match: { "sellerSplitData.sellerId": sellerObjectId } } // Strictly filter our seller after unwind
        ]);

        if (!orders || orders.length === 0) {
            return res.status(404).json({ success: false, message: "No eligible orders found for this cycle." });
        }
        ...
        orders.forEach(order => {
            const split = order.sellerSplitData;
            const isReturned = split.packageStatus === "Returned";
            
            // 🌟 FRONTEND IMAGE 64 MIRROR LOGIC
            const productAmt = Number(split.sellerSubtotal || 0);
            const customerPaid = Number(order.billDetails?.totalAmount || order.totalAmount || productAmt);
            const delDed = customerPaid > productAmt ? (customerPaid - productAmt) : 0;

            if (isReturned) {
                const netShare = -(productAmt + delDed);
                summary.sales -= productAmt;
                summary.delivery += delDed;
                summary.final += netShare;
                ...
                breakdown.push({
                    ...
                    amount: -productAmt,
                    comm_gst_tds: 0,
                    delivery_status: `+ ₹${delDed}`,
                    net_payable: netShare
                });
            } else {
                const platformComm = productAmt * (settings.commissionPercent / 100);
                const gstOnComm = platformComm * (settings.gstOnCommissionPercent / 100);
                const tdsOnComm = platformComm * (settings.tdsPercent / 100);
                const fees = platformComm + gstOnComm + tdsOnComm;
                
                const netShare = customerPaid - (fees + delDed);
        ...
        const newSettlement = new Settlement({
            ...
            finalPayable: Number(summary.final.toFixed(3)), // 💰 THE IMAGE 64 SYNC
            status: "Pending"
        });

        await newSettlement.save();

        // 5. THE LOCK: Update all involved orders as settled
        const orderIds = orders.map(o => o._id);
        await Order.updateMany({ _id: { $in: orderIds } }, { $set: { isSettled: true } });

        res.json({ success: true, message: "Weekly Settlement Generated Successfully! ✅", data: newSettlement });
