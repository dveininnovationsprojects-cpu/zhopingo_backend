...
const getWeightInKg = (value, unit = "") => {
  ...
  const u = unitMatch ? unitMatch[0] : "g";
  switch (true) {
    case /^(kg|kilogram|kilograms|k\.g)$/.test(u):
      return val;
    case /^(l|liter|liters|litre|litres)$/.test(u):
      return val;
    case /^(g|gram|grams|g\.m)$/.test(u):
      return val / 1000;
    case /^(ml|milliliter|milliliters)$/.test(u):
      return val / 1000;
    default:
...
/* =====================================================
    🌟 1. LIVE RATE CALCULATION (Internal Use)
===================================================== */
exports.getRealTimeRateInternal = async (
  pincode,
  weightKg,
  originPincode,
  paymentMode,
  ...
  try {
    ...
    const response = await axios.get(
      `${DELHI_BASE_URL}/api/kinko/v1/invoice/charges/.json`,
      {
        params: {
          md: "S",
          ss: "Delivered",
    ...
    const apiRate =
      response.data?.[0]?.total_amount || response.data?.[0]?.gross_amount;
    if (apiRate === undefined || apiRate === null)
...
const generateWarehouseName = (sellerDoc) => {
  ...
  const baseName = sellerDoc.shopName
    .replace(/[^a-zA-Z0-9]/g, "")
    .substring(0, 10);
...
exports.processShipmentCreation = async (orderId, sellerId) => {
  try {
    ...
    /* ================= ORDER ITEMS (IMPORTANT FOR INVOICE) ================= */
    const orderItemsPayload = [];

    for (const item of sellerItems) {
      const productDoc = await Product.findById(item.productId);

      const gstPercent = productDoc?.gstPercentage || 0;
      const taxAmount = (item.price * gstPercent) / 100;
...
exports.trackOrder = async (req, res) => {
  try {
    const { awb } = req.params;
    const response = await axios.get(
      `${DELHI_BASE_URL}/api/v1/packages/json/`,
      {
        params: { waybill: awb },
        headers: { Authorization: `Token ${DELHI_TOKEN}` },
      },
...
exports.downloadShippingLabel = async (req, res) => {
  try {
    ...
    if (response.data?.packages?.[0]?.pdf_url)
      return res.json({
        success: true,
        url: response.data.packages[0].pdf_url,
      });
...
exports.checkServiceability = async (req, res) => {
  try {
    ...
    const response = await axios.get(
      `${DELHI_BASE_URL}/c/api/pin-codes/json/`,
      {
        params: { filter_codes: pincode },
        headers: {
          Authorization: `Token ${DELHI_TOKEN}`,
    ...
    const serviceable = data.delivery_codes.some(
      (p) => !p.postal_code.remark
    );
    ...
    res.json({
      ...
      serviceable,
      pincode: pincode,
      details: data.delivery_codes[0]?.postal_code
    });
    ...
    res.status(500).json({
      success: false,
      error: err.response?.data || "Delhivery API failure"
    });
  }
};
// /* =====================================================
...
//     const waybill =
//       req.body.waybill || req.body.waybill_number || req.body.data?.waybill;
//     const rawStatus = (
//       req.body.status ||
...
// };
/* =====================================================
    📈 6. WEBHOOK (100% REAL-TIME AUTO-SYNC)
    Purpose: Automatically syncs Shipped, Delivered, Returned, Cancelled & Dates
===================================================== */
exports.handleDelhiveryWebhook = async (req, res) => {
  try {
    ...
    const order = await Order.findOne({
      $or: [
        ...
        { "sellerSplitData.returnAwbNumber": waybill }
      ]
    });
    ...
    if (order) {
      ...
      if (newStatus) {
                 ...
                 axios.get(`https://track.delhivery.com/api/pvt/shipping/pod.json`, {
                      ...
                      if(podRes.data && podRes.data[waybill] && podRes.data[waybill].pod_link) {
                          await Order.updateOne(
                              { _id: order._id, "sellerSplitData.awbNumber": waybill },
                              { $set: { "sellerSplitData.$.podLink": podRes.data[waybill].pod_link } }
                          );
                      }
                  }).catch(e => console.error("⚠️ Background POD Fetch Error:", e.message));
               }
            }
          }
        });
        ...
        await order.save();
        console.log(`✅ REAL-TIME SYNC SUCCESS: Order ${order._id} updated to ${newStatus}`);
      }
    }
  ...
  try {
    ...
    try {
      ...
      response = await axios.post(
        `${DELHI_BASE_URL}/api/backend/clientwarehouse/edit/`,
        payload,
        {
          headers: {
            Authorization: `Token ${DELHI_TOKEN}`,
      ...
      response = await axios.post(
        `${DELHI_BASE_URL}/api/backend/clientwarehouse/create/`,
        payload,
        {
          headers: {
            Authorization: `Token ${DELHI_TOKEN}`,
...
exports.manualRegisterWarehouse = async (req, res) => {
  try {
    ...
    const uniqueName = (
      sellerDoc.shopName.replace(/[^a-zA-Z0-9]/g, "") +
      sellerDoc._id.toString().slice(-4)
    ).substring(0, 30);
...
exports.testAwbGeneration = async (req, res) => {
  try {
    ...
    const pickupLocationName = (
      sellerDoc.shopName.replace(/[^a-zA-Z0-9]/g, "").substring(0, 10) +
      sellerDoc._id.toString().slice(-4)
    ).substring(0, 30);
...
exports.getManifest = async (req, res) => {
  try {
    ...
    const response = await axios.post(
      "https://track.delhivery.com/api/p/manifest",
      payload,
      {
        headers: {
          Authorization: `Token ${process.env.DELHIVERY_TOKEN}`,
    ...
    if (response.data?.packages?.length > 0) {
      return res.json({
        success: true,
        manifestUrl: response.data.packages[0].manifest_url
      });
    }

    return res.json({
      success: false,
      message: "Manifest not generated yet"
    });
...
exports.syncOrderStatusManual = async (req, res) => {
  try {
    const { awb } = req.body;
    const response = await axios.get(
      `${DELHI_BASE_URL}/api/v1/packages/json/?waybill=${awb}`,
      {
        headers: { Authorization: `Token ${DELHI_TOKEN}` },
      },
    );
    ...
    const latestStatus =
      response.data?.ShipmentData?.[0]?.Shipment?.Status?.Status;
    if (!latestStatus)
      return res.status(404).json({ success: false, message: "AWB not found" });
...
exports.schedulePickup = async (req, res) => {
  try {
    ...
    const payload = {
      ...
      pickup_type: "Regular"//if not working delete this 
    };

    console.log(
      `📡 Requesting Pickup for: ${pickupLocationName} on ${todayIST}`,
    );

    const response = await axios.post(PICKUP_URL, payload, {
      headers: {
        Authorization: `Token ${DELHI_TOKEN}`,
        "Content-Type": "application/json",
      },
...
exports.cancelShipment = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: "Shipment Cancel Requested",
      data: response.data
    });
...
exports.downloadDocuments = async (req, res) => {
  try {
    ...
    const response = await axios.get(
      `https://track.delhivery.com/api/pod.json`,
      {
        params: { wbns: awb },
        headers: {
          Authorization: `Token ${process.env.DELHIVERY_TOKEN}`
    ...
    console.error("Document Error:", err.response?.data || err.message);
    res.status(500).json({
      success: false,
      error: err.response?.data || err.message
    });
...
exports.createReversePickup = async (req, res) => {
  try {
    const { name, phone, address, pincode, orderId, sellerWarehouse } = req.body;
    ...
    res.json({
      success: true,
      message: "Reverse Pickup Created",
      data: response.data
    });

  } catch (err) {
    console.error("RVP Error:", err.response?.data || err.message);
    res.status(500).json({
      success: false,
      error: err.response?.data || err.message
    });
...
/* =====================================================
    ✏️ EDIT SHIPMENT (Update Details before dispatch)
===================================================== */
exports.editShipment = async (req, res) => {
  try {
    const { awb, phone, name, address } = req.body;
    ...
    console.error("Edit Shipment Error:", err.response?.data || err.message);
    res.status(500).json({
      success: false,
      error: err.response?.data || err.message
    });
...
/* =====================================================
    ⚖️ HELPER: CANCEL SHIPMENT INTERNAL (Order Controller Use panna)
===================================================== */
exports.cancelShipmentInternal = async (awb) => {
    try {
        await axios.post(
            `${DELHI_BASE_URL}/api/p/edit`,
            { waybill: awb, cancellation: "true" },
            { headers: { Authorization: `Token ${DELHI_TOKEN}`, "Content-Type": "application/json" } }
        );
        console.log(`✅ Internal Logistics Cancelled for AWB: ${awb}`);
        return true;
    } catch (e) {
        console.error(`⚠️ Internal Logistics Cancel Error:`, e.response?.data?.message || e.message);
        return false;
    }
};
/* =====================================================
    ⚖️ HELPER: CREATE REVERSE PICKUP INTERNAL (Real-Time Sync)
    Purpose: Used by Order Controller to trigger RVP in Delhivery API
===================================================== */
exports.createReversePickupInternal = async (orderId, sellerId) => {
    try {
        const order = await Order.findById(orderId);
        const sellerDoc = await Seller.findById(sellerId);
        if (!order || !sellerDoc) return { success: false, message: "Order/Seller not found" };

        // Auto-generate unique warehouse name matching dashboard sync
        const uniqueName = (sellerDoc.shopName.replace(/[^a-zA-Z0-9]/g, "").substring(0, 10) + sellerDoc._id.toString().slice(-4)).substring(0, 30);
        ...
        const payload = {
            shipments: [{
                ...
                phone: order.shippingAddress?.phone || "9876543210",
                order: order._id.toString(),
                payment_mode: "Pickup", // 🌟 Industrial Standard for RVP
                weight: 0.5 
            }],
            pickup_location: { name: uniqueName }
        };
        ...
        return { success: false, message: response.data?.data?.message || "Delhivery API Rejected" };
    } catch (err) {
        console.error(`❌ Internal RVP Sync Error:`, err.response?.data || err.message);
        return { success: false, error: err.message };
    }
};