...
// 🌟 THE SYNC: Logistics functions import
const {
  getRealTimeRateInternal,
  getWeightInKg,
  processShipmentCreation // 👈 Master shipment trigger
} = require("./logisticsController");
...
exports.createOrder = async (req, res) => {
  try {
    ...
    if (paymentMethod === "WALLET") {
      if (user.walletBalance < finalGrandTotal) {
        // Rollback Stock if wallet fails
        for (const item of processedItems) {
          await Product.findByIdAndUpdate(item.productId, {
            $inc: { stock: item.quantity },
          });
      ...
      // Since wallet deduction is successful, mark it Paid & Placed
      newOrder.status = "Placed"; 
      newOrder.paymentStatus = "Paid"; 

      // 🚀 THE MASTER SYNC TRIGGER: Wallet-ku payment success, so AWB generate pannanum
      for (let split of finalSellerSplitData) {
        try {
           ...
           if(shipmentRes.success) {
              console.log(`✅ Wallet AWB Success for ${split.shopName}: ${shipmentRes.awb}`);
              // DB-la update aagirukkum internally through processShipmentCreation
           }
        } catch (shipErr) {
           console.error(`❌ Shipment Auto-Trigger Error: ${shipErr.message}`);
        }
...
exports.getMyOrders = async (req, res) => {
  try {
    ...
    orders.forEach((order) => {
      const orderObj = order.toObject();
      const uniqueSellers = [...new Set(orderObj.items.map((item) => item.sellerId?._id?.toString() || item.sellerId?.toString()))];
      uniqueSellers.forEach((sId) => {
        const sellerItems = orderObj.items.filter((item) => (item.sellerId?._id?.toString() || item.sellerId?.toString()) === sId);
        if (sellerItems.length > 0) {
          splittedOrders.push({
            ...orderObj,
            items: sellerItems,
      ...
      uniqueSellers.forEach((sId) => {
        ...
        if (sellerItems.length > 0) {
          ...
          splittedOrdersList.push({
            ...
            items: sellerItems,
            seller: sellerItems[0].sellerId || { shopName: "Zhopingo Store" },
            billDetails: { itemTotal: sellerSubtotal, deliveryCharge: actualDeliveryCharge, totalAmount: sellerSubtotal + actualDeliveryCharge },
            totalAmount: sellerSubtotal + actualDeliveryCharge,
          });
        }
      });
...
exports.getSellerOrders = async (req, res) => {
  try {
    const { sellerId } = req.params;
    const orders = await Order.find({ "items.sellerId": sellerId }).populate("items.productId").populate({ path: "items.sellerId", select: "shopName name address city" }).sort({ createdAt: -1 });
    const splittedOrders = orders.map((order) => {
      const orderObj = order.toObject();
      const myItems = orderObj.items.filter((item) => (item.sellerId?._id?.toString() || item.sellerId?.toString()) === sellerId);
      ...
      return { ...orderObj, items: myItems, seller: myItems[0]?.sellerId || { shopName: "Merchant" }, billDetails: { itemTotal: sellerSubtotal, deliveryCharge: actualDelivery, totalAmount: sellerSubtotal + actualDelivery }, totalAmount: sellerSubtotal + actualDelivery };
    });
    res.json({ success: true, data: splittedOrders });
  } catch (err) { res.status(500).json({ success: false, error: err.message }); }
};
...
exports.requestReturn = async (req, res) => {
  try {
    ...
    order.items.forEach((item) => {
      if (item.sellerId.toString() === sellerId) {
        item.itemStatus = "Return Requested"; item.itemAwbNumber = returnAWB;
        item.isReturned = true; item.returnReason = reason; sellerItemsFound = true;
      }
    });
    if (!sellerItemsFound) return res.status(400).json({ success: false, message: "Invalid seller" });
    await order.save();
    res.json({ success: true, message: "Return request processed", returnAWB, data: order });
  } catch (err) { res.status(500).json({ success: false, error: err.message }); }
};
...
exports.getSellerReturnRequests = async (req, res) => {
  try {
    const { sellerId } = req.params;
    const orders = await Order.find({ items: { $elemMatch: { sellerId: sellerId, itemStatus: "Return Requested" } } }).populate("items.productId").populate("customerId", "name phone").sort({ updatedAt: -1 });
    const results = orders.map((order) => {
      const orderObj = order.toObject();
      orderObj.items = orderObj.items.filter((item) => item.sellerId.toString() === sellerId && item.itemStatus === "Return Requested");
      return orderObj;
    });
    res.json({ success: true, data: results });
  } catch (err) { res.status(500).json({ success: false, error: err.message }); }
  ...
  try {
    ...
    for (let split of order.sellerSplitData) {
      if (split.sellerId.toString() === sellerId?.toString()) {
        ...
        split.packageStatus = status; split.adminRemarks = adminNote || split.adminRemarks;
        if (awbNumber) split.awbNumber = awbNumber;
        if (status === "Delivered") split.deliveredDate = new Date();
        if (status === "Returned") split.returnDate = new Date();
        sellerPackageFound = true;
    ...
    if (!sellerPackageFound) return res.status(400).json({ success: false, message: "Seller not found." });

    for (let item of order.items) {
      if (item.sellerId.toString() === sellerId?.toString()) {
        if (status === "Returned" && item.itemStatus !== "Returned") {
          totalRefundAmount += item.price * item.quantity;
          item.isReturned = true; item.returnProcessedDate = new Date();
          itemsToRestore.push({ productId: item.productId, qty: item.quantity });
        }
        item.itemStatus = status; if (awbNumber) item.itemAwbNumber = awbNumber;
      }
    }
    ...
    if (status === "Returned" && totalRefundAmount > 0) {
      ...
      if (user) {
        user.walletBalance += totalRefundAmount;
        user.walletTransactions.unshift({ amount: totalRefundAmount, type: "CREDIT", reason: `Refund Approved`, date: new Date() });
        await user.save();
        for (let restore of itemsToRestore) { await Product.findByIdAndUpdate(restore.productId, { $inc: { stock: restore.qty }, $set: { status: "active" } }); }
        const allSettled = order.items.every((i) => i.itemStatus === "Returned" || i.itemStatus === "Cancelled");
        if (allSettled) order.paymentStatus = "Refunded";
      }
    }
...
exports.cancelOrder = async (req, res) => {
  try {
    ...
    if (sellerSplitIndex === -1) return res.status(400).json({ success: false, message: "Seller removed." });
    let refundAmount = 0;
    order.items.forEach((item) => {
      if (item.sellerId.toString() === sellerId?.toString()) {
        item.itemStatus = "Cancelled"; refundAmount += item.price * item.quantity;
      }
    });
    if (order.paymentStatus === "Paid") {
      const user = await User.findById(order.customerId);
      if (user) {
        user.walletBalance += refundAmount;
        user.walletTransactions.unshift({ amount: refundAmount, type: "CREDIT", reason: `Split Refund`, date: new Date() });
        await user.save();
      }
    }
    order.sellerSplitData.splice(sellerSplitIndex, 1);
    const allStatuses = order.items.map((i) => i.itemStatus);
    if (allStatuses.every((s) => s === "Cancelled")) { order.status = "Cancelled"; order.paymentStatus = "Refunded"; }
    else { const remainingActive = order.items.some((i) => i.itemStatus === "Shipped" || i.itemStatus === "Delivered"); order.status = remainingActive ? "Shipped" : "Placed"; }
    await order.save();
